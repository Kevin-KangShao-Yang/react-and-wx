 ...
 */
module.exports.setup = function(app,adminloginFunc,callback) {
	// 用户名密码 登录策略
	passport.use(new LocalStrategy(
		(username, password, done) => {
			if(!adminloginFunc) return done("登录验证函数未设置");
			adminloginFunc(username,password,function(err,user) {
...
module.exports.login = function(req,res,next) {
	...
	passport.authenticate('local', function(err, user, info) {
		...
		// 获取角色信息
		var token = jwt.sign({"uid":user.id,"rid":user.rid,"isMgr":true}, jwt_config.get("secretKey"), {"expiresIn": jwt_config.get("expiresIn")});
...
module.exports.tokenAuth = function(req,res,next) {
	passport.authenticate('bearer', { session: false },function(err,tokenData) {
		...
		if(!tokenData["isMgr"] || tokenData["isMgr"] !== true) return res.sendResult(null,400,"必须后台管理员登录")
