                ...
                <ul>
                    {this.state.fruitList.map(item => {
                        return <li key={item.id} className={styles.item}>
                            ...
                            <p>商品名：{item.name}</p>
                            <p>￥{item.price}</p>
                            <Button onClick={() => this.props.add(item)} style={{backgroundColor:'#67c23a',borderColor:'#67c23a',marginTop:10}} type="primary">加入购物车</Button>
                        </li>
                    })}
                </ul>
            </div>
...
/**
 * 
 * @param {*} dispatch 就是之前的 store.dispatch
 */
