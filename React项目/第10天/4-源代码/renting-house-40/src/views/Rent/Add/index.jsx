...
class RentAdd extends Component {
  ...
  publishHouse = async () => {
    ...
    if (size.trim().length === 0) {
      Toast.info('请输入建筑面积~', 0.8)
      return
    }

    if (!roomType) {
      Toast.info('请选择房屋类型~', 0.8)
      return
    }

    if (!floor) {
      Toast.info('请选择楼层~', 0.8)
      return
    }
    ...
    const result = await this.http.post('/houses/image', formData, {
      headers: {
    ...
    const res = await this.http.post('/user/houses', data)
